...
fun NavGraphBuilder.addLoadingNavGraph(navController: NavHostController) {
  navigation(startDestination = DestinationRoute.Loading.route, route = BaseRoute.Loading.route) {
    composable(
        DestinationRoute.Loading.route,
        deepLinks =
            listOf(navDeepLink { uriPattern = "https://lasta.jerem.ch/activity/{activityId}" }),
        arguments = listOf(navArgument("activityId") { type = NavType.StringType })) { entry ->
          ...
          LoadingScreen(
              ...
              navigateWhenLoggedIn = {
                ...
                // If the app is launched with a deep link, we navigate to the activity screen with
                // the activityId and then we transmit it to the more info screen
                navController.navigate(
                    BaseRoute.Main.route +
                        if (activityId.isEmpty()) "/DEFAULT" else "/$activityId") {
