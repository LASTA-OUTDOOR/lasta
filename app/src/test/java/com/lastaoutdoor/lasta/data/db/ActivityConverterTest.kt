...
class ActivityConverterTest {
    ...
    fun setUp(){
        ...
        fakeTrail=ActivitiesDatabaseType.Trail(activityId = 1, avgSpeedInKMH = 2.0, caloriesBurned = 3, distanceInMeters = 4, elevationChangeInMeters = 5, timeStarted = Date(6), timeFinished = Date(8))
        fakeClimb=ActivitiesDatabaseType.Climb(activityId = 0, elevationGainedInMeters = 1, numberOfPitches = 2, timeStarted = Date(3), timeFinished = Date(4))
        hashForTrail = hashMapOf(
            "activityId" to 1L,
    ...
    fun `databaseToActivity works as intended with hiking`(){
        val input =activityConverter.databaseToActivity(hashForTrail,ActivitiesDatabaseType.Sports.HIKING)
        assertEquals(fakeTrail, input)
    }

    @Test
    fun `databaseToActivity works as intended with climbing`(){
        val input =activityConverter.databaseToActivity(hashForClimb,ActivitiesDatabaseType.Sports.CLIMBING)
